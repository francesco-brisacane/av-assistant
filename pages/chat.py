...
def get_user_profiles(email):
    try:
        ...
        print(f"DEBUG: Error loading user profiles from Firebase: {e}")
    return []
...
def apply_sidebar_hiding_css():
    st.markdown("""
        <style>
        /* HIDE DEFAULT SIDEBAR PAGES MENU */
...
if not st.session_state.messages:
    ...
    if saved_data:
        ...
        if st.session_state.get('last_expert_id') == saved_data.get("expert_id"):
...
lang_code = query_params.get("lang", st.session_state.get("lang", "EN")).upper()
if lang_code not in ["IT", "EN"]: lang_code = "EN"
st.session_state["lang"] = lang_code # Save into session_state for other pages
...
with st.sidebar:
    ...
    if "code" in st.query_params and not is_logged_in:
        st.sidebar.warning(ui_text.get("auth_in_progress", "Autenticazione in corso... ricarica se bloccato."))
        if st.sidebar.button(ui_text.get("force_reload", "Ricarica Forza")):
            st.rerun()

    if not is_logged_in:
        # We need to compute I18N here. `lang_code` is already set.
        current_i18n = I18N.get(lang_code, I18N["EN"])
        login_text = current_i18n.get("login_with_google", "🔑 Login with Google")
        if lang_code == "IT":
             login_text = "🔑 Login con Google"
             
        if st.button(login_text, use_container_width=True):
            st.login("google")
    else:
        user_email = getattr(st.user, 'email', 'User')
        
        current_i18n = I18N.get(lang_code, I18N["EN"])
        logged_as = "Logged in as:" if lang_code == "EN" else "Loggato come:"
        st.write(f"{logged_as} **{user_email}**")
        
        if "user_profiles" not in st.session_state or st.session_state.get("logged_in_email") != user_email:
            st.session_state.user_profiles = get_user_profiles(user_email)
            st.session_state.logged_in_email = user_email
            
        logout_text = "🚪 Logout"
        if st.button(logout_text, use_container_width=True):
            st.session_state.pop("user_profiles", None)
    ...
    if not can_see_nav:
        apply_sidebar_hiding_css()

    # Mappa Esperti (filtrata per autenticazione)
    expert_options = {}
    
    for exp in EXPERTS_CONFIG:
        authorized_profiles = exp.get("authorizedProfiles")
        
        can_view = False
        if not authorized_profiles:
            # Se la lista non c'è o è vuota, tutti lo vedono
            can_view = True
        elif is_logged_in:
            # Se la lista c'è, l'utente deve essere loggato e avere almeno un profilo in comune
            if any(p in authorized_profiles for p in user_profiles):
                can_view = True
...
# --- 5.1 FUNZIONE SALVATAGGIO PROFILED CHAT ---
def log_profiled_chat():
    # 1. Deve essere un utente loggato
    user_email = getattr(st.user, 'email', None)
    if not user_email:
        return
        
    # 2. L'esperto deve avere profili autorizzati non vuoti
    auth_profiles = current_expert.get("authorizedProfiles", [])
    if not auth_profiles:
        return
        
    # 3. La chat deve avere almeno un messaggio
    if not st.session_state.messages:
        return
        
    chat_id = st.session_state.current_chat_id
    
    # Formattiamo i messaggi secondo le specifiche richieste
    formatted_msgs = []
    first_msg_date = st.session_state.messages[0].get("date", datetime.datetime.now().isoformat())
    
    for m in st.session_state.messages:
        formatted_msgs.append({
            "data": m.get("date", datetime.datetime.now().isoformat()),
            "ruolo": m.get("role"),
    ...
    try:
        db.collection(PROFILED_CHATS_COLLECTION).document(chat_id).set(payload)
    except Exception as e:
        print(f"DEBUG: Error saving to profiled_chats: {e}")
...
if prompt := st.chat_input(placeholder):
    user_msg_date = datetime.datetime.now().isoformat()
    st.session_state.messages.append({"role": "user", "content": prompt, "date": user_msg_date})
    ...
    with st.chat_message("assistant"):
        ...
        try:
            ...
            ast_msg_date = datetime.datetime.now().isoformat()
            st.session_state.messages.append({"role": "assistant", "content": full_res, "date": ast_msg_date})
